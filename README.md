# Vigenere Cipher Project

## Overview
This project implements the **Vigenere Cipher**, a classical encryption technique.  
It can **encrypt and decrypt** text while preserving the **case of letters** (uppercase/lowercase) and ignoring non-alphabetic characters.  

It also supports **reading from and writing to files** (`input.txt` and `output.txt`) or manual input.

---

## Features
- Encrypt and decrypt text manually or from a file.
- Preserve uppercase and lowercase letters.
- Ignore numbers, symbols, and spaces.
- Validate key to allow only alphabetic characters.
- Save the result to `output.txt`.

---

## How to Use

1. Run the program:
```bash
python vigenere_cipher.py
